...
{
    return size::controlHeight + size::controlHeightSm + space::xs;
}
...
void PreferencesRow::paint (juce::Graphics& g)
{
    ...
    g.drawFittedText (tr (entry.description), area.removeFromTop (size::controlHeightSm),
...
HostedPage::HostedPage (std::unique_ptr<juce::Component> p, int width, int height)
    : panel (std::move (p))
    , panelWidth (width)
    , panelHeight (height)
{
...
void HostedPage::resized()
{
    ...
    // At its own width, left-aligned. Stretching a panel laid out for 420 across
    // a 520 pane spreads its label column away from its controls.
    panel->setBounds (getLocalBounds().removeFromLeft (panelWidth).withHeight (panelHeight));
}
