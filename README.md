...
Then **Demos → Pulse Code** in the menu bar. There are ten, and each one is a piece of
music in a real genre rather than an illustration of a feature:

| Demo | | | What it is |
|---|---|---|---|
| Pulse Code | 132 bpm · 4/4 | 64 bars | Techno. Four drums on one insert, a pumping curve on the two beside it, a master chain ending in a limiter |
| Copper Wire | 134 bpm · 4/4 | 48 bars | UK garage. Twelve steps to a beat, because a shuffle has to be in the grid; an LFO on the hat locked to the tempo |
| Late Rhodes | 78 bpm · 4/4 | 40 bars | Neo-soul. The FM matrix used the way an electric piano uses it, over rootless voicings and a tritone substitution |
| Chrome Coast | 108 bpm · 4/4 | 48 bars | Synthwave. A wavetable position that steps rather than slides, and one muted lane holding a rejected lead |
| Slow Grain | 140 bpm · 4/4 | 48 bars | Half-time trap. Eight steps to a beat for the rolls, an 808 whose pitch drop uses the note-on latch, a riser off the bar line |
| Sublevel | 174 bpm · 4/4 | 64 bars | Drum and bass. A Reese built from FM feedback on the matrix diagonal, filtered on the channel rather than the desk |
| Iron Meter | 294 bpm · 7/8 | 56 bars | Math rock. A kit grouped 3+2+2 against a guitar grouped 2+2+3, and an accelerando into the last riff |
| Tidal Lock | 120 bpm · 5/4 | 56 bars | Berlin school. An eight-note figure inside a twenty-step bar, so it phases; FM bells and a climbing delay |
| Halcyon | 208 bpm · 6/8 | 48 bars | Ambient. Two wavetable pads morphing from different sources, one under an envelope and one under a free LFO |
| Nightfall Waltz | 96 bpm · 3/4 | 48 bars | Chamber music. Three-voice counterpoint, a canon at the fourth, and a ritardando drawn on the tempo |

A tempo counts BEATS, so in a compound or odd metre it counts the eighths: Halcyon's 208
is a dotted quarter of about 69, and Iron Meter's 294 is a quarter of 147.

Four of them get their notes from a `.score` beside them and their sound from
`src/model/demos/`, which is the language's own division: it owns notes, patterns and
...
No demo is named after a feature, and that is the point. A library with a demo per
feature demonstrates each thing once and everything else not at all — which is how the FM
matrix, the per-slot LFO, four of the ten effect types, three automation scopes and every
metre but 4/4 came to ship with nothing that used them. What holds the coverage now is a
test over the whole library rather than a demo per capability, so a track can be rewritten
for musical reasons without anything having to be renamed.
...
```sh
dew_render examples/pulse-code.dew out.wav               # the arrangement, plus its tail
dew_render examples/pulse-code.dew out.wav --bars 5:9 --normalize
dew_render examples/pulse-code.dew out.mp3 --format mp3
dew_render examples/pulse-code.dew --stems stems/        # one file per mixer track
...
```sh
ctest --preset release        # 1686 tests
```
...
fifth one, and compiled with the button there or Command-R; `examples/rhodes.score` is the
one CI compiles and renders, and `dew_score` is the same compiler on the command line.
Four scores ship, one behind each of the demos that is written rather than programmed.
