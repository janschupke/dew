...
export default function ScoreLanguage() {
  return (
    <div className="py-xxl">
      <h1 className="text-h1 text-primary font-semibold">{t('score.title')}</h1>
      <Lead>{t('score.lead')}</Lead>

      <Score name="amber" lines={[7, 30]} caption={t('score.exampleCaption')} />

      <Section title="No expressions, and that is the point">
        <p className="text-prose text-secondary max-w-[68ch]">{t('score.noExpressions')}</p>
      </Section>

      <Section title="Durations">
        <p className="text-prose text-secondary max-w-[68ch]">
          ...
          <code className="text-code-small font-mono">xN</code> is a share of what is left over and
          anything else is an exact length — different token kinds, so the two cannot be confused.
        </p>

        <p className="mt-lg text-prose text-secondary max-w-[68ch]">
          <code className="text-code-small font-mono">|</code> is a bar-line <em>assertion</em>,
          checked once the shares are known. It is the single highest-value error catcher in the
          syntax, because it turns “the section length changed and everything shifted” into one
          message.
        </p>
      ...
      <Section title="Chords">
        <p className="text-prose text-secondary max-w-[68ch]">
          ...
          inversion and <code className="text-code-small font-mono">/</code> carries tonicisation —
          using <code className="text-code-small font-mono">^</code> for the inversion is what frees{' '}
          <code className="text-code-small font-mono">/</code> for{' '}
      ...
      <Section title="What it cannot say">
        <p className="text-prose text-secondary max-w-[68ch]">
          ...
          project is not expressible — and the compiler says so in those words rather than as a
          grammar error. A note’s position is a whole number of steps, so the grid is the least
          common multiple of what the durations need: sixteenths and eighth-note triplets meet at
          twelve, and a thirty-second against any triplet needs twenty-four and is refused, naming{' '}
          <em>both</em> durations, because either alone would have been fine.
        </p>
      </Section>

      <p className="mt-xxl text-body">
