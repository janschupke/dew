...
TEST_CASE ("dragging in the velocity lane sets velocity", "[ui][pianoroll]")
{
    const juce::ScopedJuceInitialiser_GUI juceInit;
    RollHarness h;
    ...
    // Near the bottom of the lane is quiet, near the top is loud.
    const auto lane = h.roll.getVelocityArea();
    const auto x = (int) h.roll.getBoundsForNote (note).getCentreX();

    h.roll.mouseDown (eventAt (h.roll, { x, lane.getBottom() - 4 }));
    h.roll.mouseUp (eventAt (h.roll, { x, lane.getBottom() - 4 }));
    const auto quiet = (double) note[ids::velocity];

    h.roll.mouseDown (eventAt (h.roll, { x, lane.getY() + 2 }));
    h.roll.mouseUp (eventAt (h.roll, { x, lane.getY() + 2 }));
    const auto loud = (double) note[ids::velocity];

    INFO ("quiet " << quiet << " loud " << loud);
    REQUIRE (quiet < 0.2);
    REQUIRE (quiet >= 0.05);       // never silent, which would read as a bug
    REQUIRE (loud > 0.9);

    // A note at a different step is not touched by a drag over this one.
    auto other = ProjectEdits::addNote (h.pattern(), 1, 9, 1, 72, 0.5f, &undo);
    h.roll.mouseDown (eventAt (h.roll, { x, lane.getY() + 2 }));
    h.roll.mouseUp (eventAt (h.roll, { x, lane.getY() + 2 }));
    REQUIRE ((double) other[ids::velocity] < 0.6);
}
