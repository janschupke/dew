...
OscSettings classicSlot (Waveform wave = Waveform::saw, float gain = 0.8f)
{
    OscSettings s;
    s.enabled = true;
    s.wave = wave;
...
OscSettings withLfo (OscSettings s, float toPitch, float toVolume, float toPan, float hz = 5.0f)
{
    s.lfoOn = true;
    s.lfoHz = hz;
...
juce::AudioBuffer<float> renderMono (const OscBankSnapshot& bank, int numSamples = 8192)
{
...
*/
juce::AudioBuffer<float> renderInBlocks (const OscBankSnapshot& bank, int numSamples = 16384,
                                         int blockSize = 256)
{
    SynthChannel channel;
    channel.prepare (44100.0);
